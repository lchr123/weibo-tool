...
if __name__ == '__main__':
    fan_id_list = []
    for i in range(1, 40):
        page_data = get_fan_list(i)
        page_size = len(page_data['users'])
        for j in range(page_size):
            fan_id_list.append(page_data['users'][j]['id'])
    write_txt(fan_id_list)